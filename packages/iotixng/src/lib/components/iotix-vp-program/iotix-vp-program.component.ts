...
import { IotixVpTextEditorComponent } from '../iotix-vp-text-editor/iotix-vp-text-editor.component';
...
export class IotixVpProgramComponent implements OnInit, OnDestroy {
  ...
  @Output() onProgramChange = new EventEmitter<IoTiXVPProgram>();

  @Input() settings: IoTiXVPSettings = defaultSettings;

  public undoList: string[] = [];

  public redoList: string[] = [];

  @ViewChild(IotixVpTextEditorComponent) textEditor: IotixVpTextEditorComponent;
  ...
  onBlockChange(newBlock: IoTiXVPBlock) {
  ...
  checkLanguage(): IoTiXVPLanguage | undefined {
    try {
